                ...
                <List w='100%' id='scroll-bar' maxH={450} overflow='auto'>
                    {student?.packages?.length ?
                        student.packages.map(pkg => {
                            ...
                            return <VStack align={'stretch'} key={pkg._id}>
                                {pkg.package?.courses?.length ?
                                    pkg.package.courses.map(crc => {
                                        ...
                                        let totalCount = course ? _.sum([course.countTexts, course.countVideos, course.countAudios, course.countDocs]) : 0
                                        ...
                                        return (
                                            myCourse &&
                                            <div key={crc}>
                                                <CourseCard validityExpired={validityExpired} crc={course} expired={expired} track={track} />
                                            </div>
                                        )
                                    }
                                    )
...
const CourseCard = ({ crc, expired, track, validityExpired }) => {
    ...
    return (
        <ListItem pos='relative' boxShadow='0px 3px 6px #0000000A' px={6} py={2}>
            {expired ?
